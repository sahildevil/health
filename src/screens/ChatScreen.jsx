...
const ChatScreen = () => {
  ...
  const sendMessage = () => {
    ...
    // Local message to display immediately
    const localMessage = {
      id: `local-${Date.now()}`,
    ...
    setMessages(prevMessages => [localMessage, ...prevMessages]);
    
    // Also update chat history
    setChatHistory(prev => ({
      ...prev,
      [roomId]: [localMessage, ...(prev[roomId] || [])]
    }));
  ...
  const updateMessageStatus = (confirmedMessage) => {
    setMessages(prevMessages => 
      prevMessages.map(msg => {
        // Replace local pending message with confirmed one
        if (msg.pending && msg.text === confirmedMessage.text && 
            msg.senderId === confirmedMessage.senderId && 
            msg.receiverId === confirmedMessage.receiverId) {
          return { ...confirmedMessage, pending: false };
        }
        return msg;
      })
    );
    ...
    setChatHistory(prev => {
      const roomMessages = prev[confirmedMessage.roomId] || [];
      ...
      return {
        ...prev,
        [confirmedMessage.roomId]: updatedRoomMessages
      };
    });
  };
...
// // Update these URLs to match your server configuration
// const SOCKET_URL = 'http://192.168.1.9:5000'; // Updated to match your socketServer port
// const API_URL = 'http://192.168.1.9:5000'; // Updated to match your socketServer port
...
//       // Transform data if needed to match your UI expectations
//       const formattedMessages = response.data.map(msg => ({
//         id: msg.id,
//         text: msg.content || msg.text,
//         senderId: msg.sender_id || msg.senderId,
...
//     } catch (error) {
//       console.error('Error fetching message history:', error);
//       // Continue even if message history fetch fails
//     }
//   };
...
//   const sendMessage = () => {
//     if (newMessage.trim().length === 0 || !selectedDoctor || !socketConnected) {
//       if (!socketConnected) {
//         Alert.alert('Not Connected', 'Waiting for connection to server');
//       }
//       return;
//     }
...
//       // Also try to send via HTTP as a fallback if socket is unreliable
//       if (!socketConnected) {
//         sendMessageViaHttp(messageData);
//       }
//     } catch (error) {
...
//       // Add the message to the state manually since we won't get it back via socket
...
//               style={[
//                 styles.sendButton,
//                 (!newMessage.trim() || !socketConnected) && styles.sendButtonDisabled
//               ]}
//               onPress={sendMessage}
//               disabled={!newMessage.trim()}
//             >
